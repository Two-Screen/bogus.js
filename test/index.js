...
test("Render With Nested Model", function() {
  ...
  is(s, "test baz test", "basic traversal works.");
});

test("Render With Array Of Models", function() {
  ...
  is(s, "test foobar test", "basic iteration works.");
});

test("Render With Collection", function() {
  var text = "test {{#list}}foo {{val}} {{/list}}test";
  var t = Bogus.compile(text);
  var c = new Backbone.Collection([{val:'bar'}, {val:'baz'}]);
  var s = t.render({list:c});
  is(s, "test foo bar foo baz test", "basic iteration works.");
});
...
test("Single Char Delimiter", function() {
  ...
  is(s, '(bar It worked!)', "Bogus substitution worked after custom delimiters.");
});
...
test("Default content rendered inside included templates", function(){
  var include = Bogus.compile("{{$foo}}default content{{/foo}}");
...
test("Override one substitution but not the other", function() {
  var partial = Bogus.compile("{{$stuff}}default one{{/stuff}}, {{$stuff2}}default two{{/stuff2}}");
  var template = "{{<partial}}{{$stuff2}}override two{{/stuff2}}{{/partial}}";
  var t = Bogus.compile(template);
  var s = t.render({}, {"partial": partial});
  is(s, 'default one, override two', 'overrides only one substitution');

  var partial2 = Bogus.compile("{{$stuff}}new default one{{/stuff}}, {{$stuff2}}new default two{{/stuff2}}");
...
test("Super templates behave identically to partials when called with no parameters", function() {
  var partial = Bogus.compile("{{$foo}}default content{{/foo}}");
  var t = Bogus.compile("{{>include}}|{{<include}}{{/include}}");
...
test("Recursion in inherited templates", function() {
  var include = Bogus.compile("{{$foo}}default content{{/foo}} {{$bar}}{{<include2}}{{/include2}}{{/bar}}");
  var include2 = Bogus.compile("{{$foo}}include2 default content{{/foo}} {{<include}}{{$bar}}don't recurse{{/bar}}{{/include}}");
  var t = Bogus.compile("{{<include}}{{$foo}}override{{/foo}}{{/include}}");
...
test("Allows text inside a super tag, but ignores it", function() {
  var partial = Bogus.compile("{{$foo}}default content{{/foo}}");
  var t = Bogus.compile("{{<include}} asdfasd asdfasdfasdf {{/include}}");
  var s = t.render({}, {include: partial});
...
test("Ignores text inside super templates, but does parse $ tags", function() {
  var partial = Bogus.compile("{{$foo}}default content{{/foo}}");
  var t = Bogus.compile("{{<include}} asdfasd {{$foo}}hmm{{/foo}} asdfasdfasdf {{/include}}");
  var s = t.render({}, {include: partial});
...
test("Stringified templates survive a round trip", function() {
  ...
  var superTemplate = Bogus.compile("super template ");
  var include = Bogus.compile("the include ");
...
$.each(['list'], function(i, name) {
  ...
  asyncTest("Render Output: " + name, function() {
    $.when(
      ...
      var r = Bogus.compile(tmpl[0]).render({});
